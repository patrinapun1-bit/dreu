Student: Patrina Pun

Mentor: Dr. Puspita Chatterjee

# Week 2

**Dates:** 07-08 to 07-15

## Goals

-Continue working on building basic concepts of quantum computing/coding from week 1 to week 5 notebook assignments 

- Read two different papers and critically analyze them with the set of questions Jaclyn provided.
  
- Work on Assignment 7, where I will create a presentation on creating diagrams showing angle encoding, mapping 2–3 TNBC features to qubit rotations, and explaining how encoding relates to the A‑QLSTM‑A model.

- Draft Research poster

## Approach and Implementation
I continued reading various chapters of my textbook on basic quantum computing concepts. I read through the two papers that Jaclyn provided and typed in keywords such as "LSTM" or "Encoding" into the papers to narrow my reading down. I then answered the questions she provided us and critically analyzed what they meant in quantum context. 

I worked on assignment 7, which was build upon the concept on assignment 6. I utilized Qiskit and created diagrams 




## Results



## Notes


