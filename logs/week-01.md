Student: Patrina

Mentor: Dr.Puspita Chatterjee

# Week 1

**Dates:** 07/01/2026 to 07/08/2026

## Goals

- Set up my GitHub Repository and familiarize myself with the structure.

-Attend the first weekly meeting to introduce myself to the team and understand what I need to catch up on.

-Read chapter 1 of my quantum textbook and catch up on weekly notebook assignments. 

-Work on my first weekly presentation on Quantum Encoding. 



## Approach and Implementation
I communicated with Jaclyn, who invited me to clone the GitHub repository. I explored the GitHub repository on my own, followed the posted instructions, and read more about what the research is about. I attended my first weekly meeting, where I met the entire team, learned what each graduate, undergraduate, and master's student is working on, and learned about the harmonization code for a dataset we will all work on.

I also connected with another student, Khadijah, who is an undergraduate and part of the DREU program under Dr. Chatterjee. She walked me through GitHub and explained what is expected from the research internship. She briefly caught me up on the material and where to begin to get caught up. I then read a chapter from Introduction to Quantum Computing and Essential Mathematics for Quantum Computing, where I connected the concepts to the weekly notebook assignments that had been posted by Jaclyn. I completed Weeks 1–5 of the notebooks, where I learned the basics of qubits and superposition, linear algebra, quantum circuits, the Bloch sphere, noise, and the NISQ era. To learn the very basics of these concepts, I primarily read my textbook and watched various YouTube videos explaining them. I learned the concepts and also learned how to code in Qiskit.

I also attended an EDA workshop, where I learned the basic fundamentals of data cleaning. It was very informative, and we completed a short data-cleaning assignment.

I then attempted to tackle the Week 6 assignment, which was to create a presentation about how one of our harmonized dataset features can be encoded into a qubit. I approached this assignment by first understanding what quantum encoding is. I read my textbook, online websites, and academic journals. I then attended my Thursday meeting.



## Results

-Github Repo/Coding Environment(google colab, vs studio code) set up. 
- Notebooks complete, but I wish to carry this into week 2 and 3 to build upon my knowledge to understand the basics.
- Presentation complete on Quantum Encoding.
Overall, a very productive and busy first week. I am very fortunate to have gained so much knowledge in one week and will build upon all of the knowledge to continue learning new skills. 



## Notes
- Thursday Meetings at 4:30PM EST, Assignments posted every Monday.
- Continue building on basic quantum knowledge from week 1-5 notebooks. 
